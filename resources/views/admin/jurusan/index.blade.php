...
@section('content')
    ...
    <form method="GET" class="mb-6 bg-white p-4 rounded-lg shadow-sm border border-gray-200">
        <div class="flex flex-wrap items-center gap-4">
            ...
            @if ($search ?? '')
                <a href="{{ route('admin.jurusan.index') }}" class="text-gray-600 hover:text-gray-800">
                    ...
                    @forelse($jurusan as $index => $j)
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    <div
                                        class="w-8 h-8 bg-gradient-to-r from-purple-500 to-purple-600 rounded-full flex items-center justify-center mr-3">
                    ...
                    <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                        <div>
                            <p class="text-sm text-gray-700">
                                Showing <span class="font-medium">{{ $jurusan->firstItem() }}</span> to <span
                                    class="font-medium">{{ $jurusan->lastItem() }}</span> of <span
                                    class="font-medium">{{ $jurusan->total() }}</span> results
                            </p>
                        </div>
                        <div>
                            {{ $jurusan->links() }}
