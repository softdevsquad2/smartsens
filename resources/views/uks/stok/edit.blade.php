...
@section('title', 'Edit Stok - UKS')

@section('content')
    @include('uks.partials.header', [
        'title' => 'Edit Data Stok',
        'description' => 'Perbarui informasi data stok obat',
        'action' => [
    ...
    <div class="max-w-4xl mx-auto">
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <div class="bg-gradient-to-r from-amber-600 to-orange-600 px-6 py-4">
                ...
                <p class="text-orange-100 text-sm mt-1">Perbarui informasi stok obat yang dipilih</p>
            </div>

            <form action="{{ route('uks.stok.update', $stok->id_stok) }}" method="POST" class="p-6">
                @csrf
                @method('PUT')
                                ...
                                <select name="id_obat" id="id_obat"
                                    ...
                                    @foreach ($obat as $o)
                                        <option value="{{ $o->id_obat }}"
                                            {{ old('id_obat', $stok->id_obat) == $o->id_obat ? 'selected' : '' }}>
                                            {{ $o->nama_obat }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="date" name="tanggal_masuk" id="tanggal_masuk"
                                    value="{{ old('tanggal_masuk', $stok->tanggal_masuk) }}"
                                    class="block w-full pl-10 pr-3 py-3 border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500 transition-colors"
                                    required>
                            </div>
                            @error('tanggal_masuk')
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="date" name="expired_date" id="expired_date"
                                    value="{{ old('expired_date', $stok->expired_date) }}"
                                    class="block w-full pl-10 pr-3 py-3 border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500 transition-colors"
                                    required>
                            </div>
                        ...
                        <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                            ...
                            <div class="space-y-2 text-sm">
                                <div class="flex justify-between">
                                    <span class="text-gray-600">Obat:</span>
                                    <span class="font-medium text-gray-900">{{ $stok->obat->nama_obat }}</span>
                                </div>
                                <div class="flex justify-between">
                                    <span class="text-gray-600">Jumlah:</span>
                                    <span class="font-medium text-gray-900">{{ $stok->jumlah }} unit</span>
                                </div>
                                <div class="flex justify-between">
                                    <span class="text-gray-600">Tanggal Masuk:</span>
                                    <span
                                        class="font-medium text-gray-900">{{ \Carbon\Carbon::parse($stok->tanggal_masuk)->format('d/m/Y') }}</span>
                                </div>
                                <div class="flex justify-between">
                                    <span class="text-gray-600">Kadaluarsa:</span>
                                    @php
                                        $kadaluarsa = \Carbon\Carbon::parse($stok->expired_date);
                                        $now = \Carbon\Carbon::now();
                                        $isExpired = $kadaluarsa->isPast();
                                    @endphp
