    ...
    <div class="bg-white rounded-lg shadow-md p-6">
        ...
        <form action="{{ route('pelanggaran.jenis-prestasi.store') }}" method="POST" class="space-y-4">
            ...
            <div class="flex gap-3 pt-2">
                ...
                >
                    <i class="fas fa-plus mr-2"></i> Tambah Jenis Prestasi
                </button>
            </div>
        </form>
    </div>
            ...
            <tbody class="divide-y divide-gray-200">
                @forelse ($jenisPrestasi as $index => $jenis)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3 text-center">
                            <button
                                onclick="editJenisPrestasi({{ $jenis->id }}, '{{ addslashes($jenis->nama_prestasi) }}', {{ $jenis->poin_prestasi }}, '{{ addslashes($jenis->keterangan ?? '') }}')"
                                class="px-3 py-1 bg-yellow-500 text-white rounded hover:bg-yellow-600 transition-colors text-sm mr-2"
                            >
                                <i class="fas fa-edit"></i> Edit
                            </button>
                            <button
                                onclick="deleteJenisPrestasi({{ $jenis->id }})"
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-4 py-3 text-center text-gray-500">Belum ada jenis prestasi</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
...
@section('scripts')
<script>
function editJenisPrestasi(id, nama, poin, keterangan) {
...
function deleteJenisPrestasi(id) {
    Swal.fire({
